...
const i2c_port_t i2c_master_port = I2C_NUM_1;
...
void OLED::init_i2c() {
	...
	conf.sda_io_num = sda_pin;         // select GPIO specific to your project
	conf.scl_io_num = scl_pin;        // select GPIO specific to your project
	...
	ESP_LOGI("oled", "i2c before start - %d - %d", scl_pin, sda_pin);

	ESP_ERROR_CHECK(i2c_param_config(i2c_master_port, &conf));

	ESP_ERROR_CHECK(i2c_driver_install(i2c_master_port, I2C_MODE_MASTER, 32, 32, 0));

	ESP_LOGI("oled", "i2c after start");
}

void OLED::command(uint8_t adress, uint8_t c) {
	ESP_LOGI("oled", "command: %x", c);

	i2c_cmd_handle_t cmd_handle = i2c_cmd_link_create();

	ESP_ERROR_CHECK(i2c_master_start(cmd_handle));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd_handle, (adress<< 1) | I2C_MASTER_WRITE, true));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd_handle, 0x00, true)); // Co = 0, D/C = 0
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd_handle, c, true));
	ESP_ERROR_CHECK(i2c_master_stop(cmd_handle));

	ESP_ERROR_CHECK(i2c_master_cmd_begin(i2c_master_port, cmd_handle, I2C_MASTER_TOUT_CNUM_DEFAULT));
...
void OLED::data(uint8_t adress, uint8_t d) {
	...
	ESP_ERROR_CHECK(i2c_master_start(cmd_handle));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd_handle, (adress<< 1) | I2C_MASTER_WRITE, true));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd_handle, 0x40, true)); // Co = 0, D/C = 0
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd_handle, d, true));
	ESP_ERROR_CHECK(i2c_master_stop(cmd_handle));
...
void OLED::data(uint8_t adress, uint8_t *d, size_t count) {
	i2c_cmd_handle_t cmd_handle = i2c_cmd_link_create();

	i2c_master_start(cmd_handle);
	i2c_master_write_byte(cmd_handle, (adress<< 1) | I2C_MASTER_WRITE, true);
	i2c_master_write_byte(cmd_handle, 0x40, true); // Co = 0, D/C = 0
	i2c_master_write(cmd_handle, d, count, true);
...
bool OLED::init() {
	...
	ESP_LOGI("oled", "Beginning commands");
